...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.RangeSlider(id='payload-slider',min=0, max=10000, step=1250,
                                                marks={0: '0',
                                                      2500: '2500',
                                                      5000: '5000',
                                                      7500: '7500',
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='class', 
        color='class',  # Must match the column used in color_discrete_map
        color_discrete_map={
        'Success': 'lightgreen',
        'Failure': 'lightcoral'
    },
...
def get_scatter_chart(entered_site, payload_range):
    ...
    # Filter entire dataset by payload mass range first
    filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'] >= low) & 
                            (spacex_df['Payload Mass (kg)'] <= high)]

    # Then filter by site if a specific one is selected
    if entered_site == 'ALL':
        fig = px.scatter(
            filtered_df,
